---
name: voxeasy
description: VoxEasy — Vox 风格文生视频导演技能。将任意主题或文案转化为拥有地道 Vox 拼贴美学、数据驱动叙事、相邻镜头避同运镜和 1-to-1 实体映射的视频 Prompt 与脚本包，并支持标准 Vox、美漫、纪念碑谷式柔和几何、复古报纸和用户自定义全局画风。用于生成 Google Flow 等视频模型可直接使用的提示词、分镜及结构化 JSON。
---

# VoxEasy (Vox 剪纸拼贴文生视频导演技能 4.2)
...
4. **`visual_style` (全局画风预设)**:
   - `vox-standard`（默认）：经典 Vox 剪纸拼贴。
   - `american-comic`：美漫纸质拼贴；识别“美漫 / 美国漫画 / comic-book”。
   - `monument-pastel`：纪念碑谷式柔和几何画风；识别“纪念碑谷 / 柔和几何 / pastel monument”。
   - `vintage-newspaper`：复古报纸与模拟印刷；识别“复古报纸 / 旧报纸 / newsprint”。
   - `custom`：用户自定义画风；识别“自定义 / 我自己想 / 按我描述的风格 / custom style”以及任何不属于已有预设的明确画风描述。
   交互模式询问画风时，必须同时提供“标准 Vox / 美漫 / 纪念碑谷式 / 复古报纸 / 自定义（自己描述）”五个方向，不得遗漏自定义选项。
   若用户指定其中一种风格，必须读取 [`references/style-presets.md`](references/style-presets.md) 并严格使用对应规则；未指定时不得主动随机选择，继续使用 `vox-standard`。
   默认整条视频使用一种风格；若用户明确要求多风格，只能在 Shot 边界切换，并在视觉确认阶段标明每个 Shot 的 `visual_style`。
   当用户只选择 `custom` 却没有描述画风时，只问一个问题：“你想要什么画风？直接说参考方向、配色或质感即可。”用户提供一句自然语言描述后，不再要求填写表格，由 Agent 自动生成完整的 `custom_style_profile`。
5. **`target_engine` (目标渲染引擎)**: 决定 Prompt 倾向。
...
### 全局画风换皮规则 (Global Style Skin)
`visual_style` 只能改变整张画面的美术渲染语言，不能擅自重写叙事内容。风格必须同时覆盖背景、边框、标题、标签、物体、纸张表面、线条与阴影，严禁只给前景主体换风格、背景仍保留普通 Vox 画风。

除非用户明确要求重新设计画面，否则必须保留已确认镜头的构图、主体、物体数量与位置、视觉隐喻、文字、动作、时间轴和运镜。不得因为选择某种画风就自动加入该风格常见的题材或结构：
- `american-comic` 不自动增加漫画分格、对话框或英雄角色。
- `monument-pastel` 不自动增加不可能空间、连续楼梯、迷宫或游戏关卡。
- `vintage-newspaper` 不把原画面缩成一张悬浮在空白背景上的报纸。
- `custom` 不自动添加用户描述中没有出现的题材、角色、构图结构或装饰母题。

**风格介绍镜头例外**：当用户明确要求“介绍/展示某种画风”时，画风特征本身就是叙事内容，应按 [`references/style-presets.md`](references/style-presets.md) 中的“风格介绍镜头”规则实体化。尤其是美漫介绍必须展示可辨识的漫画语言，而不只是印刷纹理。
